...
from eq_cir_proxy_service.services.instrument.convert_instrument import (
    convert_instrument,
    safe_parse,
)
...
@pytest.mark.parametrize(
    "instrument_metadata, log_message",
    [
        (None, "Instrument metadata is missing"),
        ("metadata", "Instrument metadata is not a dictionary"),
        ({}, "Instrument validator_version is missing"),
    ],
)
@pytest.mark.asyncio
async def test_convert_instrument_missing_metadata(caplog, instrument_metadata, log_message):
    """Should raise 404 if instrument metadata is missing or invalid."""
    ...
    assert any(record.levelname == "ERROR" and log_message in record.message for record in caplog.records)
...
async def test_convert_instrument_lower_version_success(monkeypatch):
    ...
    monkeypatch.setattr(
        "eq_cir_proxy_service.services.instrument.convert_instrument.get_api_client",
...
def test_safe_parse_valid(monkeypatch):
    ...
    monkeypatch.setattr("eq_cir_proxy_service.services.instrument.convert_instrument.Version", DummyVersion)
