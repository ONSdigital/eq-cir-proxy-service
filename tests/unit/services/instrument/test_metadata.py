"""Unit tests for the instrument metadata retrieval service."""
...
@pytest.mark.asyncio
async def test_retrieve_instrument_metadata_success(mocker):
    """Test the retrieve_instrument_metadata function with a successful response."""
    instrument_id = uuid4()
    ...
    mock_response.json.return_value = [{"validator_version": "0.0.0"}]
    ...
    result = await retrieve_instrument_metadata(instrument_id)
    assert result == [{"validator_version": "0.0.0"}]
...
async def test_retrieve_instrument_metadata_exception(
    status_code,
    text_data,
    side_effect,
    mocker,
):
    """Test the retrieve_instrument_metadata function with various exception scenarios."""
    instrument_id = uuid4()
    ...
    with pytest.raises(HTTPException) as exc_info:
        await retrieve_instrument_metadata(instrument_id)
...
@pytest.mark.asyncio
async def test_retrieve_instrument_metadata_missing_cir_endpoint(mocker):
    """Test exception case for empty endpoint value.

    Test retrieve_instrument_metadata raises HTTPException if CIR_RETRIEVE_CI_METADATA_ENDPOINT exists
    but has no value.
    """
    instrument_id = uuid4()
    # Patch environment to include CIR_API_BASE_URL but not CIR_RETRIEVE_CI_METADATA_ENDPOINT
    mocker.patch.dict(
        os.environ,
    ...
    with pytest.raises(HTTPException) as exc_info:
        await metadata.retrieve_instrument_metadata(instrument_id)
    exc = exc_info.value
    assert exc.status_code == 500
    assert exc.detail["message"] == "CIR_RETRIEVE_CI_METADATA_ENDPOINT configuration is missing."
