...
async def test_retrieve_instrument_metadata_success(mocker):
    ...
    mock_response = make_mock_response(mocker, 200, [{"validator_version": "0.0.0"}])
...
async def test_retrieve_instrument_metadata_exception(status_code, text_data, side_effect, mocker):
    """Test exceptions in the retrieve_instrument_metadata function."""
    instrument_id = uuid4()
...
async def test_retrieve_instrument_metadata_missing_cir_endpoint(mocker):
    """Test missing endpoint configuration in the retrieve_instrument_metadata function."""
    instrument_id = uuid4()
